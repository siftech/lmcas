...
#define FIELD(NAME) obj.NAME = j.at(TOSTRING(NAME)).get<typeof(obj.NAME)>()
...
void from_json(const nlohmann::json &j, std::optional<Sigaction> &optObj) {
  if (!j.is_null()) {
    ...
    obj.sa_handler_ = j.at("sa_handler").get<typeof(obj.sa_handler_)>();
    FIELD(sa_flags);
    FIELD(sa_restorer);
    FIELD(sa_mask);
...
void from_json(const nlohmann::json &j, SyscallSetsockopt &obj) {
  ...
  FIELD(optname);
  FIELD(optlen);
  obj.optval = j.at("optval").get<typeof(string_as_array)>();
}
...
void from_json(const nlohmann::json &j, SyscallPrlimit &obj) {
  FIELD(pid);
  FIELD(resource);
  obj.newlimit = j.at("nlim").get<typeof(obj.newlimit)>();
  obj.oldlimit = j.at("olim").get<typeof(obj.oldlimit)>();
}
